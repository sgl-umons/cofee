...
def clone_repository(url: str, directory: os.PathLike = None, checkout=False) -> Union[git.Repo, None]:
...
git.Repo: The repository if it was cloned successfully, None otherwise.
    ...
    try:
        repo = git.Repo.clone_from(url, directory, **{"no-checkout": not checkout, "filter": "blob:none"})
    except git.exc.GitCommandError as e:
        logger.error("Could not clone repository at '%s'", url, exc_info=e)
        raise
    return repo
...
r (git.Repo): The repository to update.
    ...
    repo.git.fetch(**{"filter": "blob:none"})